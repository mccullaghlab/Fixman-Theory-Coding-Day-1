...
for x in range(nXyz):
	...
	for y in range(nXyz):
		...
		for z in range(nXyz):
			shift[2] = z*delta
			# rotate - but this time we will assign a random value for theta
			rotTheta = np.random.rand()*2*pi
			atomPositions = rot_x(oldPositions,rotTheta)
			rotTheta = np.random.rand()*2*pi
